...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            files1 = files_
            title = '@Filmykeedha  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1['file_name'].split()))
            size=get_size(files1['file_size'])
            f_caption=files1['caption']
            if f_caption is None:
                f_caption = f"@Filmykeedha  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1['file_name'].split()))}"
            if cd["update_channel_link"] != None:
                up = cd["update_channel_link"]
                button = [[
...
@Client.on_message(filters.command("settingsF") & filters.private)
...
@Client.on_message(filters.command("resetF") & filters.private)
...
@Client.on_message(filters.command("statsF") & filters.private)
