    ...
    async def forward_message(client, message: Message):
        try:
            ...
            if forward_type == "f":
                ...
                logger.info(f"Message {message.message_id} forwarded with tag to {to_channels}.")
            elif forward_type == "c":
                await message.copy(to_channels)
                logger.info(f"Message {message.message_id} copied without tag to {to_channels}.")
            elif forward_type == "pf":
                # Implement privacy mode forwarding here
                await message.forward(to_channels)
                logger.info(f"Message {message.message_id} forwarded with privacy mode to {to_channels}.")
            elif forward_type == "cf":
                # Implement privacy mode forwarding here
                await message.copy(to_channels)
                logger.info(f"Message {message.message_id} copied with privacy mode to {to_channels}.")
...
async def frwd_stats_command(client, message: Message):
    if message.from_user.id not in ADMINS:
        ...
        logger.warning(f"User {message.from_user.id} tried to access /frwd_stats without permission.")
        return
    ...
    if forward_data:
        ...
        logger.info(f"Admin {message.from_user.id} requested forwarding stats.")
    else:
        await message.reply("No forwarding data found.")
        logger.info(f"No forwarding data found for admin {message.from_user.id}.")
