...
#except:
#pass
search = FRESH.get(key, "").replace(' ', '_') if FRESH.get(key) else ""
sea = ""
...
seas2 = "season 01" if seas == "season 1" else "season 02" if seas == "season 2" else "season 03" if seas == "season 3" else "season 04" if seas == "season 4" else "season 05" if seas == "season 5" else "season 06" if seas == "season 6" else "season 07" if seas == "season 7" else "season 08" if seas == "season 8" else "season 09" if seas == "season 9" else "s10"
...
if not files:
await query.answer("🚫 𝗡𝗼 𝗙𝗶𝗹𝗲 𝗪𝗲𝗿𝗲 𝗙𝗼𝘂𝗻𝗱 Try S01 Series 🚫", show_alert=1)
return
temp.GETALL[key] = files
settings = await get_settings(message.chat.id)
pre = 'filep' if settings['file_secure'] else 'file'
...
btn.append(
[InlineKeyboardButton(text="This is The Last Page 🤩",callback_data="pages")]
)
if lang != "homepage":
req = query.from_user.id
offset = 0
...
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start=short_{file_id}")
return
else:
await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
else:
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start={ident}_{file_id}")
...
if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
if SHORTLINK_MODE == True:
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start=sendfiles1_{key}")

else:
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start=allfiles_{key}")
elif settings['is_shortlink'] and await db.has_premium_access(query.from_user.id):
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start=allfiles_{key}")
return
else:
...
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start=sendfiles3_{key}")
except Exception as e:
#await send_error_log(client, "1497", e)
logger.exception(e)
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start=sendfiles4_{key}")
...
if f_caption is None:
f_caption = f"{files['file_name']}"
await query.answer(url=f"https://telegram.me/rashmika_mandanana_bot?start=file_{file_id}")
...
await query.answer(url=f"https://t.me/rashmika_mandanana_bot?start={kk}_{file_id}")
...
# Additional buttons like "Google it"
btn2 = [[
InlineKeyboardButton("Join Our Offer Zone 🤑", url=OFR_CNL),
], [
InlineKeyboardButton("Search Gʀᴏᴜᴘ Lɪɴᴋ", url=GRP_LNK)
...
user = await client.get_users(from_user)
await query.answer(f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ !\nCheck Spelling and search again", show_alert=True)
else:
await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
...
if int(query.from_user.id) == int(from_user):
user = await client.get_users(from_user)
await query.answer(f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uɴᴀᴠᴀɪʟᴀʙʟᴇ !\May be spelling is wrong Or Not Available On Internet.", show_alert=True)
else:
await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
...
quote=True,
disable_web_page_preview=True,
reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('💳 Dᴏɴᴀᴛᴇ', callback_data='donation'),  # we download Link
InlineKeyboardButton("Join Our Offer Zone 🤑", url=OFR_CNL)]])  # web stream Link
)
button = [[
InlineKeyboardButton("Join Our Offer Zone 🤑", url=OFR_CNL)
...
for file in files:
cap += f"<b>\n📁 <a href='https://telegram.me/rashmika_mandanana_bot?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {clean_file_name(file['file_name'])}\n</a></b>"
else:
if settings["button"]:
...
for file in files:
cap += f"<b>📁 <a href='https://telegram.me/rashmika_mandanana_bot?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {clean_file_name(file['file_name'])}\n\n</a></b>"
...
async def safe_edit_text(msg, new_text, **kwargs):
"""
Edit message text only if the new text is different.
